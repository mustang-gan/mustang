...
class GaussianCircularToyDataLoader(DataLoader):
    """
    ...
    def __init__(self, use_batch=True, batch_size=100, n_batches=0, shuffle=False):
        super().__init__(GaussianCircularToyDataSet, use_batch, batch_size, n_batches, shuffle)
...
class GaussianCircularToyDataSet(Dataset):
    ...
    def points():
        ...
        for i in range(N_DISTRIBUTIONS):
            centers.append(
                (np.cos(2 * i * np.pi / N_DISTRIBUTIONS), np.sin(2 * i * np.pi / N_DISTRIBUTIONS)))
    ...
    def save_images(self, tensor, filename, discriminator=None):
        ...
        if not isinstance(tensor, list):
            ...
            if GaussianCircularToyDataSet.colors is None:
                GaussianCircularToyDataSet.colors = [np.random.rand(3, ) for _ in tensor]
            ...
            for i, element in enumerate(tensor):
                ...
                x, y = np.split(data, 2, axis=1)
                ax1.scatter(x.flatten(), y.flatten(), color=GaussianCircularToyDataSet.colors[i],
